...
export const getFormattedMetadata = (metaKeys: MetaKeys) => {
    let convertedMetadata: Partial<any> = {};

    for (const key in metaKeys) {
        if (metaKeys.hasOwnProperty(key)) {
            switch (key) {
                // numbers
                case 'record_length':
                ...
                case 'targets_ids_list':
                    convertedMetadata[key] = metaKeys[key].split(', ').filter(item => item.length > 0);
                    break;
                ...
                // SignatureTypes
                case 'signature_type':
                    convertedMetadata[key] = SignatureTypes[metaKeys[key] as keyof typeof SignatureTypes];
                    break;

                // SonarSystem
                case 'sonar_system':
                case 'sonogram_type':
                    convertedMetadata[key] = SonarSystem[metaKeys[key] as keyof typeof SonarSystem];
                    break;

                // FeaturesList
                case 'notable_features':
                    convertedMetadata[key] = FeaturesList[metaKeys[key] as keyof typeof FeaturesList];
                    break;
